...
class captchaSolver(Captcha):
    ...
    def checkErrorStatus(response):
        if response.status_code in [500, 502]:
            raise CaptchaServiceUnavailable(
                'anticaptcha: Server Side Error %d' % response.status_code
            )
    ...
    def requestSolve(self, captchaType, url, siteKey):
        ...
        if self.proxy:
            ...
            data['task']['type'] = "%sProxyless" % data['task']['type']
        ...
        try:
            ...
            try:
                ...
                raise CaptchaTimeout(
                    ...
                    "reporting the task with task id %d." % taskID
                )

            raise CaptchaTimeout(
                "anticaptcha: Captcha solve took to long to execute "
                "task id %d, aborting." % taskID
            )
